...
const Header = () => {
  ...
  return (
    <div className="absolute w-screen px-8 py-2 bg-gradient-to-b from-black z-10 flex flex-col md:flex-row justify-between">
      ...
      {user && (
        <div className="flex pl-0 p-2 justify-between">
          {showGptSearch && (
            <select
          ...
          <img
            className="hidden md:block w-12 h-12 rounded-md mt-2  "
            src={user?.photoURL}
            alt="usericon"
          />
          <button
            onClick={handleSignOut}
            className="font-bold  py-2 px-4 mx-2 md:mx-4  my-2 bg-red-600 text-white rounded-lg hover:bg-opacity-60"
          >
            Sign Out
          </button>
        </div>
